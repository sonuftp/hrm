...
<div class="leave-form">
	...
	if(in_array(USER_ROLE, array(SUPERADMIN_ROLE_ALIAS, ADMIN_ROLE_ALIAS))){
       	echo $form->field($model, 'user_id')->dropDownList(ArrayHelper::map(User::find()->all(),'id','email'));
		}else{
			//echo $form->field($model, 'user_id')->dropDownList(ArrayHelper::map(User::find()->where(['id'=>94])->all(),'id','email'), array('disabled' => 'disabled'));
    ...
    echo  $form->field($model, 'status')->dropDownList($getstatus);} ?>
